...
import auto_testcase_generation.cfg.testpath.IFullTestpath;
...
import auto_testcase_generation.normalizer.UnaryNormalizer;
...
public class SymbolicExecution implements ISymbolicExecution {
	 ...
	 */
	private Parameter paramaters = null;
	...
	public SymbolicExecution(List<ICfgNode> cfgNodes, Parameter paramaters, IFunctionNode functionNode)
			throws Exception {
		if (functionNode != null && paramaters != null && paramaters.size() > 0 && cfgNodes.size() >0) {
			...
			// this.addZ3Casting();
			SE(cfgNodes, this.tableMapping, functionNode);
...
//		functionConfig.setLeastSizeOfArray(5);
//		functionConfig.setMaximumInterationsForEachLoop(1);
	...
	private void symbolicTestpath(ITestpathInCFG testpath, VariableNodeTable table, IFunctionNode function)
			throws Exception {
		AbstractAutomatedTestdataGeneration.numOfSymbolicExecutions++;
		...
		INormalizedTestpath normalizedTestpath;
		...
		SE(normalizedTestpath.getAllCfgNodes(), table, function);

	}

	private void SE(List<ICfgNode> cfgNodes, VariableNodeTable table, IFunctionNode function) {
		int scopeLevel = 1;
		int count = 0;

//		this.addBoundConstraintOfArguments(constraints, table);
...
//		 */
//		if (solvingStrategy.equals( "Z3 strategy")) {
...
//
//		} else if (solvingStrategy.equals("User-bound strategy")) {
//			/*
//			 * "Z3 strategy"
//			 */
...
//	private void addBoundConstraintOfArguments(PathConstraints constraints, VariableNodeTable tableMapping) {
//		for (ISymbolicVariable var : tableMapping)
//			if (var.getScopeLevel() == ISymbolicVariable.GLOBAL_SCOPE) {
//				String type = var.getType();
//				BoundOfDataTypes dataTypes = Environment.getInstance().getBoundOfDataTypes();
//				PrimitiveBound bound = dataTypes.getBounds().get(type);
//
//				if (bound != null) {
//					String nameVarInConstraint = ISymbolicVariable.PREFIX_SYMBOLIC_VALUE + var.getName();
//					String newConstraint = nameVarInConstraint + ">=" + bound.getLowerAsLong() + "&&" + nameVarInConstraint + "<=" + bound.getUpperAsLong();
//					constraints.add(new PathConstraint(newConstraint, null, PathConstraint.ADDITIONAL_TYPE));
//				}
//			}
//	}
...
//		String lower = this.getBound(var, fn)[0];
//		String upper = this.getBound(var, fn)[1];
//
...
//                String realType = Utils.getRealType(par.getReducedRawType(), par.getParent());
				...
				if (VariableTypes.isChBasic(realType))
					...
					((OneLevelCharacterSymbolicVariable) v)
							.setSize(this.functionNode.getFunctionConfig().getBoundOfArray().getLower() + "");
	 ...
	 * @return
	 */
	private String[] getBound(ISymbolicVariable var, IFunctionNode fn) {
		String lower = IFunctionConfigBound.MIN_VARIABLE_TYPE;
		String upper = IFunctionConfigBound.MAX_VARIABLE_TYPE;
		if (var instanceof NumberSymbolicVariable || var instanceof OneDimensionNumberSymbolicVariable
			...
			lower = "0";//fnConfig.getIntegerBound().getLower();
			upper = "1";//fnConfig.getIntegerBound().getUpper();
			...
			if (VariableTypes.isBool(var.getType())) {
				...
				|| var instanceof TwoLevelCharacterSymbolicVariable) {
			lower = "0";//fnConfig.getCharacterBound().getLower();
			upper = "1";//fnConfig.getCharacterBound().getUpper();
		}
	...
	private boolean parseSingleCondition(ICfgNode stm, IASTNode ast, int count, VariableNodeTable table)
		...
		if (ast instanceof ICPPASTLiteralExpression || ast instanceof IASTIdExpression) {
			...
			if (right.equals("NULL")) {
				...
				if (r instanceof OneLevelSymbolicVariable) {
